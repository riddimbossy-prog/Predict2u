/* Predict2U v287 — Bankers page reads Stats2Pitch banker-totals-v1. */
(function(){
  'use strict';
  ...
  const S2P_URL='https://olfoahrqvwrpfesfgjqk.supabase.co/functions/v1/stats2pitch-bankers';
  const S2P_KEY='sb_publishable_xle6S4IZ_djboDdfGfG4Rg_HXPYENZY';
  const ZONE='Africa/Accra';
  const state={window:'today',league:'all',safe:[],value:[],engine:'banker-totals-v1',generatedAt:null,error:null,loading:true};
  ...
  function whyText(pick){
    if(Array.isArray(pick.reasons)&&pick.reasons.length)return pick.reasons.join(' ');
    if(Array.isArray(pick.why)&&pick.why.length)return pick.why.join(' ');
    return String(pick.whyText||pick.why||'Published by Stats2Pitch banker-totals-v1.');
  }
  ...
  function asRow(pick,lane){
    ...
    return {
      ...
      booked:true,
      engine:pick.engine||'banker-totals-v1',
      rule:pick.rule||''
    };
  }

  async function loadDate(date){
    const res=await fetch(`${S2P_URL}?date=${encodeURIComponent(date)}`,{
      headers:{apikey:S2P_KEY,Authorization:`Bearer ${S2P_KEY}`},
      cache:'no-store'
    });
    if(!res.ok)throw new Error('Stats2Pitch bankers '+res.status);
    return res.json();
  }
  ...
  async function loadBoard(){
    ...
    try{
      ...
      let engine='banker-totals-v1',generatedAt=null;
      for(const pack of packs){
        if(!pack)continue;
        engine=pack.dailyBankersMeta?.engine||pack.meta?.dailyBankersEngine||engine;
      ...
      state.loading=false;
      state.error=error&&error.message?error.message:'Stats2Pitch bankers are unavailable.';
      render();
    }
  }
    ...
    return `<article class="p2u-bankers-card is-${row.lane}">
      <div class="p2u-bankers-card-top">
        ...
        <div class="p2u-bankers-price">
          <b>${fmt(row.odds)}</b>
          <em>STATS2PITCH · ${row.lane==='safe'?'SAFE':'VALUE'}</em>
        </div>
      </div>
  ...
  function render(){
    ...
    if($('bankers-meta')){
      if(state.loading)$('bankers-meta').textContent='Reading Stats2Pitch bankers…';
      else if(state.error)$('bankers-meta').textContent=state.error;
      else $('bankers-meta').textContent=`Stats2Pitch ${state.engine} · ${safe.length} safest · ${value.length} value · updated ${stamp}`;
    }
    if($('safe-acca'))$('safe-acca').textContent=acca(safe);
    if($('value-acca'))$('value-acca').textContent=acca(value);
    if($('safe-list')){
      if(state.loading)$('safe-list').innerHTML='<div class="p2u-bankers-empty">Reading Stats2Pitch safest bankers…</div>';
      else if(state.error)$('safe-list').innerHTML=`<div class="p2u-bankers-empty">${esc(state.error)}</div>`;
      else $('safe-list').innerHTML=safe.length?safe.map((row,i)=>card(row,i)).join(''):'<div class="p2u-bankers-empty">No Stats2Pitch safest banker in this window.</div>';
    }
    if($('value-list')){
      if(state.loading)$('value-list').innerHTML='<div class="p2u-bankers-empty">Reading Stats2Pitch value bankers…</div>';
      else if(state.error)$('value-list').innerHTML=`<div class="p2u-bankers-empty">${esc(state.error)}</div>`;
      else $('value-list').innerHTML=value.length?value.map((row,i)=>card(row,i)).join(''):'<div class="p2u-bankers-empty">No Stats2Pitch value banker in this window.</div>';
    }
    const registry={};
    ...
    document.querySelectorAll('[data-banker-slip]').forEach(btn=>{
      btn.onclick=()=>{
        ...
        window.P2USlip.add(row.match,row.canonical||row.market,'Stats2Pitch Bankers');
    ...
    document.querySelectorAll('[data-banker-acca]').forEach(btn=>{
      btn.onclick=()=>{
        ...
        pack.forEach(row=>window.P2USlip.add(row.match,row.canonical||row.market,'Stats2Pitch Bankers'));
        if(typeof window.P2USlip.open==='function')window.P2USlip.open();
      };
    });
  }
