...
(function(){
  ...
  async function getComments(articleId){
    const sb=await getClient();if(!sb)return[];
    let result=await sb.from(COMMENTS).select('id,user_id,handle_snapshot,body,status,created_at').eq('article_id',articleId).eq('status','visible').order('created_at',{ascending:true}).limit(250);
  ...
  function friendlyCommentError(error){
    ...
    if(/moderation_status|column .* does not exist|schema cache|function .* does not exist/i.test(raw))return 'Comments need the one-time v198 database repair. Ask the site owner to run the included SQL file.';
  ...
  function bind(){
    ...
    document.addEventListener('keydown',e=>{if(e.key==='Escape')closeDiscussion();if(e.key==='Enter'&&(e.ctrlKey||e.metaKey)&&e.target&&e.target.id==='news-comment-body'){e.preventDefault();const submit=$('[data-news-comment-submit]');if(submit)postComment(submit.dataset.newsCommentSubmit,submit);}});
