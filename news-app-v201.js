/* Predict2U v201 — News comment reactions, ownership controls and like alerts.
   ...
   followed topics, Read Later records, reports and owner-managed community comments. */
(function(){
  'use strict';
  const VERSION='v201';
  ...
  const COMMENT_LIKES='p2u_news_comment_likes';
  ...
  const ARTICLE_CACHE_KEY='p2u-news-articles-v198';
  ...
  let follows=[],bookmarks=new Set(),lastFocused=null,discussionComments=[];
  let schemaMode='modern',lastFetchError='',searchMode='exact',commentSocialAvailable=true;
  ...
  const selectorId=v=>window.CSS&&typeof CSS.escape==='function'?CSS.escape(String(v)):String(v).replace(/[^a-zA-Z0-9_-]/g,'\\$&');
  ...
  function actionsHtml(a,featured=false){
    ...
    return `<div class="p2u-news-actions">${url?`<a class="primary p2u-news-source-action" href="${esc(url)}" target="_blank" rel="noopener noreferrer nofollow"><i class="fa-solid fa-arrow-up-right-from-square" aria-hidden="true"></i><span>Source</span></a>`:''}<button type="button" class="p2u-news-discuss-action" data-news-discuss="${esc(a.id)}"><i class="fa-regular fa-comments" aria-hidden="true"></i><span>Discuss${Number(a.comment_count||0)?` · ${Number(a.comment_count)}`:''}</span></button><button type="button" class="p2u-news-bookmark-action ${saved?'is-saved':''}" data-news-bookmark="${esc(a.id)}" aria-label="${saved?'Remove from':'Save to'} Read Later"><i class="fa-${saved?'solid':'regular'} fa-bookmark" aria-hidden="true"></i><span>${saved?'Saved':'Read later'}</span></button><button type="button" class="p2u-news-share-action" data-news-share="${esc(a.id)}" aria-label="Share story"><i class="fa-solid fa-share-nodes" aria-hidden="true"></i><span>Share</span></button><button type="button" class="p2u-news-report-action" data-news-report-article="${esc(a.id)}" aria-label="Report story"><i class="fa-regular fa-flag" aria-hidden="true"></i><span>Report</span></button></div>`;
  }
  ...
  async function getComments(articleId){
    const sb=await getClient();if(!sb)return[];
    let result=await sb.from(COMMENTS).select('id,user_id,handle_snapshot,body,status,created_at,updated_at,edited_at,like_count').eq('article_id',articleId).eq('status','visible').order('created_at',{ascending:true}).limit(250);
    if(result.error&&isSchemaError(result.error)){
      commentSocialAvailable=false;
      result=await sb.from(COMMENTS).select('id,user_id,handle_snapshot,body,status,created_at,updated_at').eq('article_id',articleId).eq('status','visible').order('created_at',{ascending:true}).limit(250);
    }
    if(result.error&&isSchemaError(result.error))result=await sb.from(COMMENTS).select('id,user_id,handle_snapshot,body,created_at').eq('article_id',articleId).order('created_at',{ascending:true}).limit(250);
    if(result.error){lastFetchError=String(result.error.message||'Comments unavailable');return[];}
    const rows=(result.data||[]).map(c=>Object.assign({like_count:0,edited_at:null,liked_by_me:false},c));
    if(session&&rows.length&&commentSocialAvailable){
      const ids=rows.map(c=>Number(c.id)).filter(Number.isFinite);
      try{
        const likes=await sb.from(COMMENT_LIKES).select('comment_id').eq('user_id',session.user.id).in('comment_id',ids);
        if(!likes.error){
          const mine=new Set((likes.data||[]).map(x=>String(x.comment_id)));
          rows.forEach(c=>{c.liked_by_me=mine.has(String(c.id));});
        }else if(isSchemaError(likes.error))commentSocialAvailable=false;
      }catch(_){}
    }
    return rows;
  }
  ...
  function commentHtml(c){
    ...
    const liked=Boolean(c.liked_by_me),count=Math.max(0,Number(c.like_count||0));
    const edited=c.edited_at?'<span class="p2u-news-comment-edited">Edited</span>':'';
    const ownerTools=own?(commentSocialAvailable?`<button type="button" class="p2u-news-comment-owner-action" data-news-comment-edit="${esc(c.id)}"><i class="fa-regular fa-pen-to-square"></i><span>Edit</span></button><button type="button" class="p2u-news-comment-owner-action danger" data-news-comment-delete="${esc(c.id)}"><i class="fa-regular fa-trash-can"></i><span>Delete</span></button>`:''):`<button type="button" data-news-report-comment="${esc(c.id)}" aria-label="Report comment"><i class="fa-regular fa-flag"></i><span class="sr-only">Report</span></button>`;
    const disabledLike=own?' disabled aria-disabled="true" title="You cannot like your own comment"':'';
    const likeAction=commentSocialAvailable?`<div class="p2u-news-comment-actions"><button type="button" class="p2u-news-comment-like ${liked?'is-liked':''}" data-news-comment-like="${esc(c.id)}" aria-pressed="${liked?'true':'false'}"${disabledLike}><i class="fa-${liked?'solid':'regular'} fa-heart"></i><span>${liked?'Liked':'Like'}</span><b data-comment-like-count>${count}</b></button></div>`:'';
    return `<article class="p2u-news-comment" data-comment-id="${esc(c.id)}"><div class="p2u-news-comment-top"><div><strong>@${esc(c.handle_snapshot||'member')}</strong> <time>${when(c.created_at)}</time> ${edited}</div><div class="p2u-news-comment-tools">${own?'<span class="p2u-news-own-label">You</span>':''}${ownerTools}</div></div><p data-comment-body>${esc(c.body)}</p>${likeAction}</article>`;
  }

  async function discussionHtml(a){
    const comments=await getComments(a.id);discussionComments=comments;
    const socialNote=commentSocialAvailable?'Like helpful comments. You can edit or delete your own posts.':'Comment likes and editing will appear after the v201 database update.';
    return `<div class="p2u-news-discussion-head"><div><div class="p2u-news-kicker">NEWS COMMUNITY</div><h2>${esc(a.title)}</h2></div><button class="p2u-news-close" data-news-close aria-label="Close">×</button></div><div class="p2u-news-meta">${metaHtml(a)}</div><div class="p2u-news-trust-note"><i class="fa-solid fa-shield-halved"></i><span>Discuss the report, not other members. Source links open on the publisher site. ${esc(socialNote)}</span></div><div class="p2u-news-comment-form">${session?`<label for="news-comment-body">Join the conversation</label><textarea id="news-comment-body" maxlength="600" placeholder="Add a respectful football comment…"></textarea><div class="p2u-news-comment-form-foot"><small>No abuse, spam, private information or repeated promotions.</small><button class="p2u-news-button" data-news-comment-submit="${esc(a.id)}">Post comment</button></div><div class="p2u-news-comment-message" aria-live="polite"></div>`:`<p>Sign in to join the discussion, like comments and manage your own posts.</p><a class="p2u-news-button" href="account.html">Open Account Center</a>`}</div><div class="p2u-news-comments-head"><h3>${comments.length} comment${comments.length===1?'':'s'}</h3><button type="button" class="p2u-news-link-button" data-news-report-article="${esc(a.id)}"><i class="fa-regular fa-flag"></i> Report story</button></div><div id="news-comments-list">${comments.length?comments.map(commentHtml).join(''):'<div class="p2u-news-empty compact">No comments yet. Start the conversation.</div>'}</div>`;
  }
  ...
  async function openDiscussion(id){
    ...
    const requestedComment=new URLSearchParams(location.search).get('comment');
    if(requestedComment){
      const target=panel.querySelector(`[data-comment-id="${selectorId(requestedComment)}"]`);
      if(target){target.classList.add('is-highlighted');setTimeout(()=>target.scrollIntoView({behavior:'smooth',block:'center'}),80);}
    }
  }
  ...
  async function resolveActiveSession(sb){
    let active=window.P2UAccounts&&window.P2UAccounts.getSession?window.P2UAccounts.getSession():session;
    if(active&&active.user)return active;
    try{const {data}=await sb.auth.getSession();active=data&&data.session||null;}catch(_){active=null;}
    session=active;return active;
  }

  function friendlyCommentError(error){
    const raw=String(error&&error.message||'');
    if(/sign in|required|jwt|auth/i.test(raw))return 'Your sign-in expired. Open Account Center, sign in again, then repost.';
    if(/duplicate/i.test(raw))return 'That comment was already posted.';
    if(/limit|rate/i.test(raw))return 'You are posting too quickly. Wait a moment and try again.';
    if(/spam/i.test(raw))return 'The comment was blocked by the spam filter. Remove promotional wording or repeated links.';
    if(/cannot like your own/i.test(raw))return 'You cannot like your own comment.';
    if(/not found or cannot be edited/i.test(raw))return 'Only the comment owner can edit this post.';
    if(/not found or cannot be deleted/i.test(raw))return 'Only the comment owner can delete this post.';
    if(/moderation_status|column .* does not exist|schema cache|function .* does not exist/i.test(raw))return 'Comment social actions need the one-time v201 database update. Ask the site owner to run the included SQL file.';
  ...
  async function postComment(articleId,button=null){
    ...
    if(message){message.textContent='Posting…';message.dataset.state='loading';}
    try{
      const {data,error}=await sb.rpc('p2u_news_post_comment',{p_article_id:String(articleId),p_body:body});
      if(error)throw error;
      if(textarea)textarea.value='';
      if(message){message.textContent='Comment posted.';message.dataset.state='success';}
      const a=articles.find(x=>String(x.id)===String(articleId));if(a)a.comment_count=Number(a.comment_count||0)+1;
      if(currentArticle){const panel=$('#news-discussion-panel');if(panel)panel.innerHTML=await discussionHtml(currentArticle);}
      applyFilter();showToast('Comment posted.');
    }catch(error){if(message){message.textContent=friendlyCommentError(error);message.dataset.state='error';}}
    finally{if(btn){btn.disabled=false;btn.textContent=btn.dataset.originalText||'Post comment';delete btn.dataset.originalText;}}
  }

  function commentById(id){return discussionComments.find(c=>String(c.id)===String(id))||null;}

  async function toggleCommentLike(commentId,button){
    if(!session){showToast('Sign in to like comments.','bad');return;}
    if(!navigator.onLine){showToast('Reconnect before liking a comment.','bad');return;}
    const sb=await getClient();const active=sb?await resolveActiveSession(sb):null;
    if(!sb||!active){showToast('Sign in to like comments.','bad');return;}
    if(button)button.disabled=true;
    try{
      const {data,error}=await sb.rpc('p2u_news_toggle_comment_like',{p_comment_id:Number(commentId)});
      if(error)throw error;
      const liked=Boolean(data&&data.liked),count=Math.max(0,Number(data&&data.like_count||0));
      const c=commentById(commentId);if(c){c.liked_by_me=liked;c.like_count=count;}
      if(button){
        button.classList.toggle('is-liked',liked);
        button.setAttribute('aria-pressed',liked?'true':'false');
        const icon=button.querySelector('i');if(icon)icon.className=`fa-${liked?'solid':'regular'} fa-heart`;
        const label=button.querySelector('span');if(label)label.textContent=liked?'Liked':'Like';
        const total=button.querySelector('[data-comment-like-count]');if(total)total.textContent=String(count);
      }
      showToast(liked?'Comment liked.':'Like removed.');
    }catch(error){showToast(friendlyCommentError(error),'bad');}
    finally{if(button)button.disabled=false;}
  }

  function startCommentEdit(commentId){
    const c=commentById(commentId),row=$(`[data-comment-id="${selectorId(commentId)}"]`);
    if(!c||!row)return;
    const body=row.querySelector('[data-comment-body]');if(!body)return;
    body.outerHTML=`<div class="p2u-news-comment-editor"><label for="news-comment-edit-${esc(commentId)}">Edit your comment</label><textarea id="news-comment-edit-${esc(commentId)}" maxlength="600">${esc(c.body)}</textarea><div class="p2u-news-comment-editor-actions"><button type="button" class="p2u-news-button" data-news-comment-save="${esc(commentId)}">Save changes</button><button type="button" class="p2u-news-ghost" data-news-comment-cancel="${esc(commentId)}">Cancel</button></div><div class="p2u-news-comment-inline-message" aria-live="polite"></div></div>`;
    row.querySelector('textarea')?.focus();
  }

  function cancelCommentEdit(commentId){
    const c=commentById(commentId),row=$(`[data-comment-id="${selectorId(commentId)}"]`);
    const editor=row&&row.querySelector('.p2u-news-comment-editor');if(!c||!editor)return;
    editor.outerHTML=`<p data-comment-body>${esc(c.body)}</p>`;
  }

  async function saveCommentEdit(commentId,button){
    const row=$(`[data-comment-id="${selectorId(commentId)}"]`),textarea=row&&row.querySelector('.p2u-news-comment-editor textarea'),message=row&&row.querySelector('.p2u-news-comment-inline-message');
    const body=clean(textarea&&textarea.value,600);
    if(body.length<2){if(message)message.textContent='Comment must be at least 2 characters.';return;}
    if(!navigator.onLine){if(message)message.textContent='Reconnect before editing this comment.';return;}
    const sb=await getClient();const active=sb?await resolveActiveSession(sb):null;if(!sb||!active){if(message)message.textContent='Sign in again before editing.';return;}
    if(button){button.disabled=true;button.textContent='Saving…';}
    try{
      const {data,error}=await sb.rpc('p2u_news_edit_comment',{p_comment_id:Number(commentId),p_body:body});
      if(error)throw error;
      const c=commentById(commentId);if(c){c.body=data&&data.body||body;c.edited_at=data&&data.edited_at||new Date().toISOString();}
      if(currentArticle){const panel=$('#news-discussion-panel');if(panel)panel.innerHTML=await discussionHtml(currentArticle);}
      showToast('Comment updated.');
    }catch(error){if(message)message.textContent=friendlyCommentError(error);}
  ...
  async function deleteComment(commentId,button){
    ...
    if(!navigator.onLine){showToast('Reconnect before deleting this comment.','bad');return;}
    const sb=await getClient();const active=sb?await resolveActiveSession(sb):null;if(!sb||!active){showToast('Sign in again before deleting.','bad');return;}
    if(button)button.disabled=true;
    try{
      const {data,error}=await sb.rpc('p2u_news_delete_comment',{p_comment_id:Number(commentId)});
      if(error)throw error;
      discussionComments=discussionComments.filter(c=>String(c.id)!==String(commentId));
      const a=articles.find(x=>String(x.id)===String(data&&data.article_id||currentArticle&&currentArticle.id));
      if(a)a.comment_count=Math.max(0,Number(a.comment_count||0)-1);
      if(currentArticle){const panel=$('#news-discussion-panel');if(panel)panel.innerHTML=await discussionHtml(currentArticle);}
      applyFilter();showToast('Comment deleted.');
    }catch(error){showToast(friendlyCommentError(error),'bad');}
    finally{if(button)button.disabled=false;}
  }
  ...
  function bind(){
    ...
    document.addEventListener('click',e=>{
      ...
      const likeComment=e.target.closest('[data-news-comment-like]');if(likeComment){toggleCommentLike(likeComment.dataset.newsCommentLike,likeComment);return;}
      const editComment=e.target.closest('[data-news-comment-edit]');if(editComment){startCommentEdit(editComment.dataset.newsCommentEdit);return;}
      const saveComment=e.target.closest('[data-news-comment-save]');if(saveComment){saveCommentEdit(saveComment.dataset.newsCommentSave,saveComment);return;}
      const cancelComment=e.target.closest('[data-news-comment-cancel]');if(cancelComment){cancelCommentEdit(cancelComment.dataset.newsCommentCancel);return;}
      const deleteCommentButton=e.target.closest('[data-news-comment-delete]');if(deleteCommentButton){deleteComment(deleteCommentButton.dataset.newsCommentDelete,deleteCommentButton);return;}
      ...
      const submit=e.target.closest('[data-news-comment-submit]');if(submit){postComment(submit.dataset.newsCommentSubmit,submit);return;}
    ...
    document.addEventListener('keydown',e=>{if(e.key==='Escape'&&!e.target.closest('.p2u-news-comment-editor'))closeDiscussion();if(e.key==='Enter'&&(e.ctrlKey||e.metaKey)&&e.target&&e.target.id==='news-comment-body'){e.preventDefault();const submit=$('[data-news-comment-submit]');if(submit)postComment(submit.dataset.newsCommentSubmit,submit);}if(e.key==='Enter'&&(e.ctrlKey||e.metaKey)&&e.target&&e.target.closest('.p2u-news-comment-editor')){e.preventDefault();const row=e.target.closest('[data-comment-id]'),save=row&&row.querySelector('[data-news-comment-save]');if(save)saveCommentEdit(save.dataset.newsCommentSave,save);}});
  ...
  async function realtime(){
    ...
    try{channel=sb.channel('p2u-news-live-v201').on('postgres_changes',{event:'INSERT',schema:'public',table:TABLE},payload=>{if(payload&&payload.new&&payload.new.published!==false){ingestNewsAlert(payload.new);fetchArticles({silent:true});}}).on('postgres_changes',{event:'*',schema:'public',table:COMMENTS},payload=>{const row=payload.new||payload.old||{};if(currentArticle&&String(row.article_id)===String(currentArticle.id))openDiscussion(currentArticle.id);}).subscribe();}catch(_){}
  }
  ...
  window.P2UNews={version:VERSION,refresh:fetchArticles,open:openDiscussion,postComment,toggleCommentLike,startCommentEdit,saveCommentEdit,deleteComment,getArticles:()=>articles.slice(),getFollows:()=>follows.slice(),getBookmarks:()=>[...bookmarks]};
