(function(){
  ...
  const links=[
    ...
    {href:'engines.html',label:'Full Board',mobileLabel:'Full',icon:'fullboard'},
  ...
  dock.innerHTML=links.map(item=>`<a href="${item.href}" class="${isActive(item)?'is-active':''}">${svg(item.icon)}<span>${item.mobileLabel||item.label}</span></a>`).join('');
