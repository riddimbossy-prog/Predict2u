...
check('service worker matches build',sw.includes(`const VERSION='${version}';`)&&sw.includes(`const CACHE_VERSION='predict2u-${version}';`));
...
const report={build:version,generated_at:new Date().toISOString(),passed:checks.length-failed.length,failed:failed.length,checks};
fs.writeFileSync(path.join(root,'active-engine-board-report-v220.json'),JSON.stringify(report,null,2));
if(failed.length){
