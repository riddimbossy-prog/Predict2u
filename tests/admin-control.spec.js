...
test('backend admin loads for an authorized owner and fits Z Fold cover', async ({page})=>{
  ...
  await expect(page.locator('#p2u-account-launcher')).toHaveCount(0);
  const metricColumns=await page.locator('.metric-grid').evaluate(el=>getComputedStyle(el).gridTemplateColumns.split(' ').length);
...
test('publishing and moderation actions call protected backend RPCs', async ({page})=>{
  ...
  await page.goto('/admin.html',{waitUntil:'domcontentloaded'});
  await waitAdmin(page);
  await page.evaluate(()=>window.P2UBackendAdmin.openTab('publishing'));
  await expect(page.locator('[data-admin-panel="publishing"]')).toBeVisible();
  await page.locator('label.switch:has(#announcement-enabled)').click();
  await expect(page.locator('#announcement-enabled')).toBeChecked();
  ...
  await page.evaluate(()=>window.P2UBackendAdmin.openTab('community'));
  await expect(page.locator('[data-admin-panel="community"]')).toBeVisible();
