/* Predict2U v266 — deterministic release gate for the stabilized product. */
'use strict';
...
const old=[];for(const f of fs.readdirSync(ROOT).filter(x=>!['release-gate-v264.js','release-gate-v265.js','release-gate-v266.js'].includes(x)&&/\.(js|json|md|yml|yaml|html|txt)$/.test(x))){const full=path.join(ROOT,f);if(fs.statSync(full).size<2e6&&read(f).includes('riddimbossy-prog/golden-banker-v2'))old.push(f);}if(old.length)errors.push(`Old repository slug remains in: ${old.join(', ')}`);
...
if(errors.length){console.error(errors.join('\n'));process.exit(1);}console.log('Predict2U v266 release gate passed.');
