(function(){
  ...
  function render(){
    ...
    host.querySelectorAll('[data-pack]').forEach(button=>button.addEventListener('click',()=>{
      const key=decodeURIComponent(button.dataset.pack);const pack=packs.find(item=>item.engine.key===key);if(!pack||!window.P2USlip)return;
      const result=P2USlip.addMany(pack.picks.map(p=>({m:p.match,market:p.market,engine:pack.engine.name})),pack.engine.name);
