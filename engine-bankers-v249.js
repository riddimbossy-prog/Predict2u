(function(){
  ...
  function render(){
    const visible=packs.filter(pack=>family==='all'||(pack.engine.familyLabel||pack.engine.family)===family);
    const familyOptions=[['all','All'],...Array.from(new Set(registry.map(e=>e.familyLabel||e.family))).map(x=>[x,x])];
    const familyButtons=familyOptions.map(([key,label])=>`<button class="p2u-engine-acca-filter${key===family?' is-active':''}" data-family="${esc(key)}">${esc(label)}</button>`).join('');
    const emptyCard=`<div class="p2u-engine-pack-empty"><strong>No engine picks today</strong>${blocked?'Current data is stale, so engine packs are paused.':'No engine passed its rules for today’s fixtures.'}</div>`;
    host.innerHTML=`<div class="p2u-engine-accas-head"><div><div class="t-lbl" style="color:var(--em)">TODAY'S ENGINE ACCAS</div><h2>Top 2–3 picks from each engine</h2><p>Each active engine gets its own branded mini-acca. Engines with fewer qualifiers show only what passed.</p></div><a href="all-engines.html" class="text-[12px] font-semibold" style="color:var(--em)">Open all engines →</a></div>
      <div class="p2u-engine-acca-filters" role="group" aria-label="Engine family filter">${familyButtons}</div>
