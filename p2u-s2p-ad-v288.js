/* Predict2U v288 — Stats2Pitch open-site promo */
(function(){
  'use strict';
  const KEY='p2u-s2p-ad-v288';
  ...
  async function rows(){
    try{
      ...
      for(const date of dates){
        const res=await fetch(FEED+'?date='+encodeURIComponent(date),{headers:{apikey:FEED_KEY,Authorization:'Bearer '+FEED_KEY},cache:'no-store'});
        const body=await res.json().catch(()=>null);
        const pack=[...(body&&body.safestBankers||[]),...(body&&body.valueBankers||[])].filter(p=>p&&p.home&&p.away);
        if(pack.length)return pack.slice(0,2).map(p=>({home:p.home,away:p.away,pick:p.displaySelection||p.pick||p.selection,odds:p.odds}));
      }
    }catch{}
    return fallback;
  }
  ...
  function mount(picks){
    ...
    wrap.className='p2u-s2p-ad-backdrop';
    wrap.innerHTML=`<article class="p2u-s2p-ad-card" role="dialog" aria-modal="true" aria-label="Stats2Pitch free picks">
      ...
      <div class="p2u-s2p-ad-rows">${picks.map(p=>`<div class="p2u-s2p-ad-row"><div><small>${esc(p.home)} vs ${esc(p.away)}</small><strong>${esc(p.pick)}</strong></div><em>${p.odds?fmt(p.odds):'FREE'}</em></div>`).join('')}</div>
