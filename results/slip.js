...
const P2USlip=(()=>{
  ...
  async function builtinSave(isPublic, msgEl){
    ...
    if(!sb){
      say('Opening sign in…');
      const next=location.pathname.split('/').pop()||'bankers.html';
      location.href='account.html?view=signin&reason=save-slip&next='+encodeURIComponent(next+location.search);
      return;
    }
    say('Saving…');
    try{
      ...
      if(!sess){
        say('Sign in is required to save this slip. Redirecting…',1);
        const next=location.pathname.split('/').pop()||'bankers.html';
        setTimeout(()=>{ location.href='account.html?view=signin&reason=save-slip&next='+encodeURIComponent(next+location.search); },250);
        return;
      }
...
// v261: engine and banker modules use window.P2USlip. Expose the shared
// slip controller explicitly so bulk-add buttons work on every page.
window.P2USlip=P2USlip;
