(function(){
  ...
  function addOne(pick){
    const slip=window.P2USlip;if(!slip)return;
    slip.add(pick.match,pick.market,engine.name);
    if(typeof slip.open==='function')slip.open();
  }
  function addPack(rows,button){
    const slip=window.P2USlip;if(!slip){button.textContent='Reload to use My Slip';return;}
    const result=slip.addMany(rows.map(row=>({m:row.match,market:row.market,engine:engine.name})),engine.name);
